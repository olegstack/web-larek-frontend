...
Этот класс реализует HTTP-запросы к серверу, предоставляя методы для получения и отправки данных, обновления информации. Все запросы выполняются с базовым URL на который будут направляться все запросы. По умолчанию используется Content-Type.

- `handleResponse` - обработка ответа от сервера, возвращает JSON или ошибку.
- `get` - выполняет GET-запрос. Возвращает промис с результатом.
- `post` - выполняет POST-запрос с данными. Возвращает промис с результатом.
